...
double speeds[NMOTORS]={0.0,0.0,0.0,0.0};// 2电机速度值 0～10
...
void updateSpeed() {   
    ...
    for (int i = 0; i < NMOTORS; ++i) {
        // 更新速度
        set_velocity_client = n->serviceClient<webots_ros::set_float>(string("/load_good_bot/") + string(motorNames[i]) + string("/set_velocity"));   
...
speeds[2]  = 10.0*(2.0*linear_temp - L*angular_temp)/2.0;
speeds[3]  = 10.0*(2.0*linear_temp + L*angular_temp)/2.0;

*/

double u = 0 ;
double v = -linear_temp ;
double delta_angle = angular_temp;
...
*/
    updateSpeed();
ROS_INFO("angular_temp:%lf,  linear_temp:%lf",angular_temp, linear_temp);
    ROS_INFO("left:%lf,  right:%lf", speeds[0], speeds[1]);
}
...
/*
    for (int i = 0; i < NMOTORS; ++i) {
        ...
        // 初始化2个控制位置的电机
        set_position_client = n->serviceClient<webots_ros::set_float>(string("/load_good_bot/") + string(motorNames[i]) + string("/set_position"));   
        set_position_srv.request.value = INFINITY;
        if (set_position_client.call(set_position_srv) && set_position_srv.response.success)     
            ROS_INFO("Position set to INFINITY for motor %s.", motorNames[i]);   
        else     
            ROS_ERROR("demo_2dnav_move Failed to call service set_position on motor %s.", motorNames[i]);   
        set_velocity_client = n->serviceClient<webots_ros::set_float>(string("/load_good_bot/") + string(motorNames[i]) + string("/set_velocity"));   
