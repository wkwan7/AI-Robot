  ...
  while (wb_robot_step(TIME_STEP) != -1)
  {
     ...
     */
      ...
      for (int i = 0; i < info.number_of_objects; i++)
      {
        ...
        pos += sprintf(buffer+pos, "%d ", objects[i].id);
        // position
        pos += sprintf(buffer+pos, "%.2f %.2f %.2f ", objects[i].position[0], objects[i].position[1], objects[i].position[2]);
        // size
        pos += sprintf(buffer+pos, "%.2f %.2f\n", objects[i].size[0], objects[i].size[1]);
      ...
      //char out[9] = "message!";
      printf("sending\n");
      wb_emitter_send(emitter, buffer, 1000);
